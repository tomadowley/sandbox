...
describe("insertionSort", () => {
  ...
  it("sorts an array with mixed positive and negative numbers", () => {
    expect(insertionSort([3, -1, 4, -2, 0])).toEqual([-2, -1, 0, 3, 4]);
  });

  it("sorts an array with repeated elements", () => {
    expect(insertionSort([2, 2, 2, 2])).toEqual([2, 2, 2, 2]);
    expect(insertionSort([3, 1, 1, 5, 5, 3])).toEqual([1, 1, 3, 3, 5, 5]);
  });

  it("sorts a large array", () => {
    const arr = Array.from({ length: 100 }, (_, i) => 100 - i);
    expect(insertionSort(arr)).toEqual(Array.from({ length: 100 }, (_, i) => i + 1));
  });
  ...
  it("sorts an array with large and small numbers", () => {
    expect(insertionSort([1000000, -1000000, 0, 42, -42])).toEqual([-1000000, -42, 0, 42, 1000000]);
  });
});